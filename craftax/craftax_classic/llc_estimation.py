...
# from craftax.craftax_classic.tree_water_experiment import generate_test_world
...
jit_gen_traj_495 = jax.jit(lambda params, rng: generate_trajectory(params, rng, num_steps=10000))
#%%
network = ActorCritic(17, layer_size)
rng, _rng = jax.random.split(rng)
...
model_no = -1
checkpoint_directory = f"/workspace/CraftaxDevinterp/intermediate"
model_list = os.listdir(checkpoint_directory)
model_file = f"{checkpoint_directory}/{model_list[model_no]}"
folder_list = os.listdir(model_file)
checkpointer = ocp.StandardCheckpointer()
network_params = checkpointer.restore(f"{model_file}/{folder_list[0]}", args=ocp.args.StandardRestore(abstract_params))
...
expert_obses = expert_obses.reshape(expert_obses.shape[0] * expert_obses.shape[1], expert_obses.shape[2])
expert_logitses = expert_logitses.reshape(expert_logitses.shape[0] * expert_logitses.shape[1], expert_logitses.shape[2])
#%%
...
# folder_list = os.listdir(model_file)
# checkpointer = ocp.StandardCheckpointer()
# network_params = checkpointer.restore(f"{model_file}/{folder_list[0]}")
...
# plt.tight_layout()
# plt.savefig(f"/workspace/CraftaxDevinterp/llc_estimation/debug/calibration.png")
...
num_models = len(os.listdir("/workspace/CraftaxDevinterp/intermediate"))
...
for model_no in tqdm(range(0, num_models, 1)):
    checkpoint_directory = f"/workspace/CraftaxDevinterp/intermediate/{checkpoint_list[model_no]}"
    checkpointer = ocp.StandardCheckpointer()
