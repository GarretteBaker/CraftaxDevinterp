...
experiment_name = "only_wood"
save_dir = f"/workspace/CraftaxDevinterp/intermediate_data/environments/{experiment_name}"
os.makedirs(save_dir, exist_ok=True)
for i in range(5):
    state_no = i * 100
    ...
    plt.title(f"{experiment_name} State {state_no}")
    plt.savefig(f"{save_dir}/{experiment_name}_state_{state_no}.png")
    plt.close()


from tqdm import tqdm
checkpointer = ocp.StandardCheckpointer()
for checkpointno in tqdm(range(1525)):
    checkpoint_directory = f"/workspace/CraftaxDevinterp/intermediate/{checkpointno}"
    folder_list = os.listdir(checkpoint_directory)
    ...
    save_dir = f"/workspace/CraftaxDevinterp/intermediate_data/modelno_{checkpointno}/{experiment_name}"
    os.makedirs(save_dir, exist_ok=True)
    for i, control_activation in enumerate(control_activations):
        u, s, v = jnp.linalg.svd(control_activation, full_matrices=False)
        plt.plot(s)
        plt.title(f"Singular Values of Activations for layer {i}, model number {checkpointno}")
        plt.savefig(f"{save_dir}/singular_values_{i}.png")
        plt.close()


    for i, control_activation in enumerate(control_activations):
        plt.imshow(control_activation, cmap="viridis")
        plt.title(f"Activations for layer {i}, model number {checkpointno}")
        plt.colorbar()
...
def create_video(input_pattern, output_file, start_number=0, end_number=1254, input_fps=24, output_fps=30):
    ffmpeg_command = [
        'ffmpeg',
        ...
        '-q:v', '1',
        '-r', str(output_fps),
        '-frames:v', str(end_number - start_number + 1),
        output_file
    ]
...
save_dir = f"../../intermediate_data/environments/{experiment_name}"
input_pattern = '../../intermediate_data/modelno_%d/' + f"{experiment_name}/activations_0.png"
output_file = f'{save_dir}/layer_0_acts.mp4'
create_video(input_pattern, output_file)

input_pattern = '../../intermediate_data/modelno_%d/' + f"{experiment_name}/activations_1.png"
output_file = f'{save_dir}/layer_1_acts.mp4'
create_video(input_pattern, output_file)

input_pattern = '../../intermediate_data/modelno_%d/' + f"{experiment_name}/activations_2.png"
output_file = f'{save_dir}/layer_2_acts.mp4'
create_video(input_pattern, output_file)
# %%
