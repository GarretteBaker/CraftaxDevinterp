    ...
    def train(rng):
        ...
        if config["SAVE_TRAJ"]:
            def count_unique(n):
                ...
                return len(jnp.unique(timing(t)))+1
            eps = 0
            l = 1600
            while count_unique(l) > 1600:
                old_l = l
                l -= int(l * 0.1)
                assert old_l != l
            u = 4000
            while count_unique(u) < 1600:
                old_u = u
                u += int(u * 0.1)
                assert old_u != u
            m = (l+u) / 2
            count = count_unique(int(m))
            time_limit = 1e4
            t=0
            while abs(count - 1600) > eps and t <= time_limit:
                if count < 1600:
                    l = m
                elif count > 1600: 
                    u = m
