...
def make_train(config):
    ...
    def train(rng):
        # INIT NETWORK
        if is_symbolic:
            network = LinearActorCritic(env.action_space(env_params).n, config["LAYER_SIZE"])
        else:
        ...
        if config["SAVE_TRAJ"]:
            ...
            for update_no in range(int(config["NUM_UPDATES"])):
                ...
                os.makedirs(f"/workspace/CraftaxDevinterp/intermediate_linear/{update_no}", exist_ok=True)
                path = ocp.test_utils.erase_and_create_empty(f"/workspace/CraftaxDevinterp/intermediate_linear/{update_no}")
